...
function mergeObjects(objects) {
  ...
  objects.forEach((item) => {
    Object.entries(item).forEach((entr) => {
      res[entr[0]] = (res[entr[0]] || 0) + entr[1];
    });
  });
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject).forEach((item) =>
    lettersObject[item].forEach((number) => {
      arr[number] = item;
    })
  );
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    let currMoneyCount = 0;
    Object.keys(bank).forEach((currItem) => {
      currMoneyCount += bank[currItem] * currItem;
    });
    if (queue[i] - currMoneyCount > 25 && i > 0) {
      return false;
    }

    bank[queue[i]] = (bank[queue[i]] || 0) + 1;
  }
...
function sortCitiesArray(arr) {
  ...
  arr.forEach((item) => {
    if (!Array.isArray(groupObj[item.country])) {
      groupObj[item.country] = [];
    }

    groupObj[item.country].push(item.city);
  });

  Object.keys(groupObj)
    .sort()
    .forEach((country) => {
      if (groupObj[country].length > 1) {
        groupObj[country].sort().forEach((city) => res.push({ country, city }));
      } else {
        res.push({ country, city: groupObj[country][0] });
      }
    });
...
function group(array, keySelector, valueSelector) {
  ...
  keys.forEach((key, index) => {
    if (map.has(key)) {
      map.set(key, [...map.get(key), values[index]]);
    } else {
